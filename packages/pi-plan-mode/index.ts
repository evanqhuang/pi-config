...
For plans, features, fixes, and other implementation work, split the work into focused, independently verifiable units and delegate those units with the Agent tool. Implementation agents are forced to the ImplementationWorker leaf-worker profile, which runs with normal full-access tools (YOLO behavior) and cannot create, launch, steer, or wait on subagents. Every implementation delegation uses model "openai-codex/gpt-5.6-luna" and thinking "xhigh". Launch independent units in parallel when their files and dependencies do not overlap. Keep tightly coupled integration and coordination in the main session. Give each subagent exact requirements, owned files, forbidden changes, and a focused verification command. Never trust a subagent summary by itself: inspect the actual changed files or diff, run relevant diagnostics and tests, and fix integration issues. After the approved plan has been implemented, use LunaCompliance to compare the finished implementation against the approved plan and user requirements, and use LunaTestVerifier to independently inspect the actual test commands, results, and coverage evidence. These dedicated read-only verifier profiles use model "openai-codex/gpt-5.6-luna" with thinking "high". Run them after implementation, in parallel when independent, then resolve every actionable gap they identify. Verify the complete result yourself before reporting completion. Do not delegate final accountability or claim success from unverified subagent output.`;
