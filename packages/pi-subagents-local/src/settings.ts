...
export interface SubagentsSettings {
   ...
   * case). Omitted fails closed; a type name explicitly opts those calls into
   * that fallback; `"none"` is the explicit strict-dispatch spelling.
   *
   * The boolean `false` is accepted as a spelling of `"none"`, because a boolean
   * would otherwise be dropped as the wrong type and silently leave the strict
   * policy implicit while the author believes strict dispatch is on. Every
   * other value is an agent name, so a mistaken `"off"` fails loudly at
   * dispatch rather than meaning one thing here and another in the resolver.
   */
...
function sanitize(raw: unknown): SubagentsSettings {
  ...
  if (r.fallbackSubagent === false) {
    ...
    // dropped, silently leaving the strict policy implicit. Every string is an
    // agent name except the `none` sentinel, which the resolver recognizes — so
    // a mistaken "off" fails loudly at dispatch instead of meaning something
