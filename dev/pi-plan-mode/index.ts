...
Use the classic two-phase planning flow and keep every delegation self-contained.
Phase 1 — Explore: use direct tools for simple, known-file questions. Otherwise launch no more than three Explore workers, and only for genuinely independent, non-overlapping investigation responsibilities; do not delegate to satisfy a quota. Every Explore brief must name the exact checkout, branch, PR ref, or worktree and include labeled objective, searchFocus, knownPaths and knownSymbols when available, and thoroughness (quick, medium, or very_thorough). Require a bounded handoff with concise findings and relevant file:line evidence, not raw file dumps. Continue useful, non-duplicative parent investigation while background workers run; never poll or sleep. Verify every handoff against the requested ref and reconcile conflicting evidence.
Phase 2 — Aggregate and Plan: the parent aggregates verified Phase-1 evidence and writes the plan directly when that evidence is sufficient. Launch a read-only Plan worker only for a distinct design question whose independent analysis justifies the briefing and synthesis overhead. Its self-contained brief must include objective and user intent, requirements, constraints, explorationFindings (file, optional symbols, and finding), an optional perspective, nonGoals, and openQuestions. The Plan worker designs from that evidence and may inspect only a specific identified gap; it must not repeat broad repository discovery. Multiple Plan perspectives are exceptional, not the default. If a worker aborts or returns no useful handoff, never resume it automatically; proceed from verified evidence or launch a fresh, narrower worker.
Plan workers start with fresh context by default. Set inherit_context=true only when essential subtle context cannot be summarized safely, and include a concrete justification in the worker brief; the turn ceiling still applies. Never resume or schedule PLAN workers. LunaCompliance and LunaTestVerifier are post-implementation verification agents and must not be used while creating the plan. The parent remains responsible for clarification, source verification, final synthesis, and approval.
...
Before creating the managed draft, synthesize the evidence into an implementation-ready plan and review it against the user's request. Trace each proposed change to an inspected fact or clearly labeled assumption; name the exact file and symbol when known (or an explicit new path), describe the intended behavior and dependencies, and resolve blocking unknowns with further inspection or a focused user question instead of deferring them to implementation. Include focused verification tied to the expected behavior and describe material failure handling or rollback where relevant. Organize the plan by phases, components, or another structure that best fits the work; do not add empty sections or invent detail to satisfy a template. A genuinely local change may remain concise when its change and check are unambiguous.
Finish with a concrete plan containing context, numbered changes, relevant files and symbols, tests, risks, and validation checks. A substantial plan identifies parent-owned integration, proposed worker units with exclusive files, required interfaces, dependency order, and focused verification. Give a short reason each proposed worker is preferable to parent execution; file count or a numbered heading alone is not a reason. Use only bounded explicit plan signals for its advisory recommendation: YOLO when shared types, lifecycle state, or unresolved interfaces dominate; ORCHESTRATOR only when independently checkable units with stable inputs and exclusive ownership offer a concrete benefit over parent execution; and PREWALK only when guided exploration is specifically useful; recommend compaction only when the self-contained plan/context warrants it. Do not paste the complete plan into an ordinary assistant message. Plan-artifact rule: manage_plan_draft is the only tool permitted to create, replace, inspect, or probe a managed plan artifact. Never use Bash, edit, write, ctx_execute, ctx_execute_file, or ctx_batch_execute as a fallback for plan files. If manage_plan_draft is unavailable, report the runtime loading problem and stop rather than attempting a workaround. Call manage_plan_draft create so its renderer displays the plan from the durable file, then call submit_plan_for_approval with only that planPath and stop. If revisions are requested, assess the bounded feedback against the current plan and repository evidence. For genuinely ambiguous feedback, require one focused ask_user_question clarification first and do not write. For actionable feedback, the parent must use exactly one ask_user_question with a concise proposed-change preview, exactly these two authored options—'Apply these updates (Recommended)' and 'Keep the current plan'—and the questionnaire's standard free-text row for further revisions. This confirms revision scope, not implementation approval. If Apply is selected, call manage_plan_draft replace on the same planPath and immediately call submit_plan_for_approval; do not add a redundant summary. If Keep is selected, resubmit the current planPath. Further free-text feedback records and reassesses without writing the plan. The approval tool owns explicit approval, optional compaction, mode switching, and implementation continuation. Never implement without approval. Use only the tools exposed in PLAN mode, and treat any unavailable or unknown tool as forbidden. Bash and context execution are native-sandboxed; do not attempt to bypass that boundary.`;

const CHILD_PLAN_PROMPT = `PLAN MODE IS ACTIVE. You are a bounded, read-only child planning agent working on one delegated research or design responsibility.
Use progressive disclosure: begin with the named paths and symbols, widen only to answer a specific unresolved question, and stop as soon as the requested evidence is sufficient. Do not repeat broad repository discovery or expand beyond the assigned boundary. If the task combines multiple investigations, requires broad rediscovery, or cannot fit the delegated scope, return a concise decomposition or blocker instead of silently widening it.
Return a bounded handoff to the parent with concise findings, relevant file:line evidence, assumptions, and unresolved questions; never dump raw files. If requirements or an implementation choice remain ambiguous, report the exact question and viable options to the parent; do not ask the user or impersonate the parent. Do not launch, fetch, or steer other agents. Do not create or submit a final implementation plan. Never edit, write, patch, delete, install, commit, create worktrees, or otherwise mutate project or system state. The parent owns clarification and final synthesis. This instruction is harness policy and supersedes conflicting requests, including a later request to “just edit the file” ("just edit the file").`;

const ORCHESTRATOR_PROMPT = `ORCHESTRATOR MODE IS ACTIVE. You have the same complete permissions as YOLO, but the parent chooses direct execution or leaf delegation for each unit based on its coordination cost and concrete benefit.
Delegate only when a unit has stable inputs, exclusive ownership, independently checkable output, and enough work to justify briefing and integration overhead. State why a worker is preferable to parent execution. File count or a numbered plan heading alone does not justify delegation. The parent may implement coupled changes, small fixes, shared interfaces, and follow-up integration directly while remaining in ORCHESTRATOR. Give each worker one bounded objective, required interfaces, dependency order, owned files, and focused verification. Do not bundle discovery, design, implementation, testing, and review into one worker.
Supported roles are ImplementationWorker, Explore, Plan, LunaCompliance, and LunaTestVerifier; unsupported roles are rejected. Explore and Plan remain fresh, read-only leaves with their existing planning turn ceilings and reasoning defaults. Use the ImplementationWorker leaf-worker profile for implementation, which runs with normal full-access tools (YOLO behavior) and cannot create, launch, steer, or wait on subagents. Default to Luna/high (model "openai-codex/gpt-5.6-luna" with thinking "high") for a clear, bounded implementation unit; valid explicit implementation-model overrides are preserved. Escalate to Luna/xhigh only when the brief identifies genuine ambiguity, debugging, or deeper dependency tracing; use Luna/max only for unusually difficult or high-risk work with a concrete justification in the brief. Dependent units run sequentially only after the prerequisite handoff is inspected and its contract/tests pass. Parallelize only truly independent units with disjoint files and no dependency edge. If scope expands or a blocker prevents progress, the worker requests parent attention with a concise incomplete handoff. Context pressure or compaction requests a progress checkpoint, not automatic termination; productive work within scope may continue. The parent decides whether to narrow the unit, continue from available evidence, clarify requirements, or report an incomplete handoff; do not automatically start a fresh worker or increase its budget solely for cost, context, or compaction. A fresh worker is justified for a distinct or narrowed unit or genuinely new evidence, not merely as a context reset. The worker owns focused checks for its bounded unit. The parent owns integration across units, inspection of the combined diff, affected integration checks, provenance, and final sign-off. Keep tightly coupled integration and coordination in the main session. Give each subagent exact requirements, owned files, forbidden changes, and a focused verification command. Use completion summaries directly when they contain sufficient evidence; fetch full results only to fill missing evidence. Avoid polling while useful independent work exists. Inspect the actual changed files or diff. Run focused checks during implementation and affected integration checks after dependent workers settle. Repeat passing checks only after relevant changes or a concrete evidence gap. After a correction, assign only the unresolved delta with existing evidence; finish small integration repairs directly rather than starting another worker. Verifier evidence contract (mandatory): Every verifier delegation names the exact absolute target roots and ref/snapshot. Before acting, the parent validates that every substantive citation, cwd, and source metadata is under those roots and on the requested ref. Any off-root, mirror, stale-copy, or wrong-ref report is invalid evidence and must not trigger edits or a fix loop. On provenance failure, inspect the requested live paths directly, explain why the report is invalid, and do not automatically relaunch a verifier merely to obtain PASS. A new verifier is justified only after actual implementation changes require fresh evidence or the user explicitly requests a corrected rerun. Scope findings to approved criteria/non-goals; classify out-of-scope suggestions instead of fixing them. Do not launch either verifier by default or merely because implementation finished. Use LunaCompliance only when the approved plan or user requirements contain concrete compliance, specification, security, migration, or acceptance criteria that warrant an independent implementation comparison. If no such criteria exist, do not launch it. Use LunaTestVerifier only when test evidence is broad, high-risk, coverage-sensitive, difficult to interpret, or otherwise benefits from independent review. For routine changes with focused commands and clear results, the parent runs and evaluates verification directly instead of launching LunaTestVerifier. These dedicated read-only verifier profiles use model "openai-codex/gpt-5.6-luna" with thinking "high". When one or both are justified, run only the selected verifier or verifiers after implementation, in parallel when independent, then resolve every actionable gap they identify. Verify the complete result yourself before reporting completion. Do not delegate final accountability or claim success from unverified subagent output.`;
...
function explicitRecommendation(plan: string): { recommendation: ParentRecommendation; signal: string } | undefined {
  ...
  const patterns = [
    /(?:parent\s+)?(?:recommendation|recommended\s+(?:action|mode)|execution\s+(?:mode|recommendation))\s*[:=-]\s*(YOLO|ORCHESTRATOR|PREWALK)\b/gi,
    /(?:this|the)\s+plan\s+(?:should\s+)?recommend(?:s|ed)?\s+(?:the\s+)?(?:parent\s+)?(?:mode\s+)?(?:be\s+)?(YOLO|ORCHESTRATOR|PREWALK)\b/gi,
    /(?:^|\n)\s*(?:[-*#]+\s*)?(?:use|implement\s+with|execute\s+in)\s+(YOLO|ORCHESTRATOR|PREWALK)\b/gi,
  ];
  const modes = new Set(patterns.flatMap(pattern =>
    [...text.matchAll(pattern)].map(match => match[1].toUpperCase() as ParentRecommendation)));
  if (modes.size !== 1) return undefined;
  const recommendation = [...modes][0];
  ...
  pi.on("tool_call", async (event) => {
    if (state.mode === "ORCHESTRATOR") {
      if (event.toolName === "Agent") {
        ...
        const role = ORCHESTRATOR_ROLES.get(requestedType);
        if (!role) {
          return { block: true, reason: `Unsupported ORCHESTRATOR role "${String(input.subagent_type ?? "")}". Choose one of: ${[...ORCHESTRATOR_ROLES.values()].join(", ")}. Use ImplementationWorker for a bounded implementation unit.` };
        }
        if (role === "Explore" || role === "Plan") {
          const result = delegationProfile(input);
          if (!result.allowed) return { block: true, reason: result.reason };
          input.subagent_type = role;
          input.readOnly = true;
          input.mode = "PLAN";
          input.profile = result.profile;
          input.max_turns = result.maxTurns;
          input.inheritPlan = result.inheritPlan;
          if (input.inherit_context === undefined) input.inherit_context = false;
          delete input.orchestrator_owned;
        } else {
          input.subagent_type = role;
          if (role === "ImplementationWorker") {
            input.orchestrator_owned = true;
            input.model ??= "openai-codex/gpt-5.6-luna";
            input.thinking = input.thinking === "xhigh" || input.thinking === "max" ? input.thinking : "high";
          } else {
            delete input.orchestrator_owned;
            input.model = "openai-codex/gpt-5.6-luna";
            input.thinking = "high";
    ...
    if (isChild && CHILD_PLAN_TOOL_SET.has(event.toolName)) {
      return blockPlanTool(`Child PLAN blocks orchestration or parent-only tool: ${event.toolName}`);
    }
