...
export interface RunOptions {
   ...
   * prompt continues it. Routing does not resolve the requested type again or
   * apply a fresh verifier policy to this historical continuation.
   */
...
export async function runAgent(
  ...
  // AgentManager resolves fresh spawns before a record is created. Keep this
  // boundary strict as well because runAgent is exported and can be called
  // directly: a missing card must never acquire general-purpose's prompt,
  // model, or tools by accident.
  const agentConfig = getAgentConfig(type);
  if (!agentConfig || agentConfig.enabled === false) {
    throw new Error(`No agent configuration available for type "${type}"`);
  }
  ...
  if (agentConfig?.memory) {
    ...
    const denied = deniedTools.length > 0 ? new Set(deniedTools) : undefined;
  ...
  const disallowedSet = disallowedTools.length > 0 ? new Set(disallowedTools) : undefined;
