...
export interface AgentConfig {
  ...
  /** How an isolated worktree is finalized after the agent exits. */
  worktreeDisposition?: WorktreeFinalization;
  /** Whether an isolated worktree receives the source checkout's changes. */
...
/**
 * Safe worktree metadata for lifecycle/history surfaces. This deliberately
 * contains refs, hashes, sizes, and bounded path lists only — never a patch or
...
export interface AgentRecord {
  ...
  /** Full cleanup metadata after agent completion. */
  worktreeResult?: CleanupResult;
...
export interface AgentInvocation {
  ...
  runInBackground?: boolean;
  isolation?: IsolationMode;
  /** Effective worktree finalization strategy, when known. */
  worktreeDisposition?: WorktreeFinalization;
  /** Effective source-checkout snapshot strategy, when known. */
  snapshotSource?: boolean;
}
...
export interface NotificationDetails {
  ...
  outputFile?: string;
  error?: string;
  /** Safe worktree metadata; raw patches and file contents are excluded. */
  worktree?: WorktreeReport;
  worktreeResult?: WorktreeReport;
