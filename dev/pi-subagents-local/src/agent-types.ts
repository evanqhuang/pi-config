 ...
 * to exactly one enabled agent. Strict dispatch is the default; an explicit
 * agent name in `subagents.json` (`fallbackSubagent`) opts into fallback.
 * `NO_FALLBACK` is the explicit spelling of the default policy.
 *
...
/** Get the configured fallback agent type. Defaults to strict dispatch. */
export function getFallbackSubagent(): string | undefined { return fallbackSubagent; }

/** Set the configured fallback agent type. An omitted/empty value restores strict dispatch. */
export function setFallbackSubagent(v: string | undefined): void {
  const value = typeof v === "string" ? v.trim() : "";
  fallbackSubagent = value || NO_FALLBACK;
}
...
export function resolveSpawnTypeIn(
  ...
  // A missing type follows the same policy as a wrong one rather than being
  // silently treated as general-purpose. Empty input is still reported
  // separately so callers get an actionable error.
  ...
  // Strict dispatch is the safe default. A caller-supplied name that did not
  // identify one enabled agent must not silently acquire the general-purpose
  // agent's model, prompt, or tool policy. The explicit fallback branch above
  // is the only way to opt into substitution.
  return { ok: false, message: `${reason} Available: ${available()}.` };
}
