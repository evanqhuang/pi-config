...
import { type AgentConfig, type AgentInvocation, type AgentMentionMode, type AgentRecord, type JoinMode, type NotificationDetails, type SubagentType, type ViewerMarkdownMode, type WidgetMode, type WorktreeReport } from "./types.js";
...
/** Build the bounded metadata exposed outside the live AgentRecord. */
function buildWorktreeMetadata(record: Pick<AgentRecord, "worktree" | "worktreeResult">): {
  worktree?: WorktreeReport;
  worktreeResult?: WorktreeReport;
} {
  const metadata: { worktree?: WorktreeReport; worktreeResult?: WorktreeReport } = {};
  const worktree = toWorktreeReport(record.worktree);
  const worktreeResult = toWorktreeReport(undefined, record.worktreeResult);
  if (worktree) metadata.worktree = worktree;
  if (worktreeResult) metadata.worktreeResult = worktreeResult;
  return metadata;
}
...
function formatTaskNotification(record: AgentRecord, resultMaxLen: number, showCost = false): string {
  ...
  const worktreeMetadata = buildWorktreeMetadata(record);
  // JSON is bounded by toWorktreeReport and then XML-escaped. This gives
  // consumers the complete strategy/ref summary without ever embedding a
  // patch or copied file content in a notification.
  const worktreeXml = worktreeMetadata.worktree
    ? `<worktree>${escapeXml(JSON.stringify(worktreeMetadata.worktree))}</worktree>`
    : "";
  const worktreeResultXml = worktreeMetadata.worktreeResult
    ? `<worktree_result>${escapeXml(JSON.stringify(worktreeMetadata.worktreeResult))}</worktree_result>`
    : "";
...
export default function (pi: ExtensionAPI) {
  ...
  const spawnResolved = (piRef: any, ctxRef: any, type: string, prompt: string, options: any) => {
    ...
    // Resume reopens an existing session and must retain its historical
    // semantics. Fresh programmatic spawns, on the other hand, use the same
    // canonical card/callsite policy as the Agent tool before reaching the
    // manager. In particular this prevents a registry/RPC/mention caller from
    // relabeling a verifier or removing its tool denylist.
    let resolvedOptions = rawOptions;
    if (!rawOptions.resumeSessionFile) {
      const invocation = resolveAgentInvocationConfig(getAgentConfig(dispatch.type), {
        model: typeof rawOptions.model === "string" ? rawOptions.model : undefined,
        thinking: rawOptions.thinking ?? rawOptions.thinkingLevel,
        max_turns: rawOptions.max_turns ?? rawOptions.maxTurns,
        run_in_background: rawOptions.run_in_background ?? rawOptions.isBackground,
        inherit_context: rawOptions.inherit_context ?? rawOptions.inheritContext,
        isolated: rawOptions.isolated,
        isolation: rawOptions.isolation,
        worktree_disposition: rawOptions.worktree_disposition ?? rawOptions.worktreeDisposition,
        snapshot_source: rawOptions.snapshot_source ?? rawOptions.snapshotSource,
      }, {
        worktreeAllowed: isWorktreeIsolationEnabled(),
        defaultRunInBackground: rawOptions.isBackground ?? true,
        agentType: dispatch.type,
      });
      if (invocation.policyError) throw new Error(invocation.policyError);

      // Keep the caller's dispatch/runtime options, replacing only the values
      // whose effective safety policy is authoritative. The invocation snapshot
      // is updated too, so registry, RPC, and @mention rows expose the same
      // strategy metadata as Agent-tool rows.
      resolvedOptions = {
        ...rawOptions,
    ...
    const { state, callbacks } = createActivityTracker(resolveEffectiveMaxTurns(dispatch.type, resolvedOptions?.maxTurns));
    ...
    const id = manager.spawn(piRef, ctxRef, dispatch.type, prompt, { ...resolvedOptions, ...callbacks });
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  const agentTool = defineTool({
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      // Resume reuses the existing session and deliberately keeps its historical
      // semantics: invocation strategy policy applies only to fresh sessions.
      if (!params.resume && resolvedConfig.policyError) return textResult(resolvedConfig.policyError);

      // Resolve model from explicit tool-call params first; the agent card fills gaps.
      let model = ctx.model;
      ...
      const worktreeDisposition = resolvedConfig.worktreeDisposition;
      const snapshotSource = resolvedConfig.snapshotSource;
      const disallowedTools = resolvedConfig.disallowedTools;
  ...
  registerToolReportingUsage(defineTool({
    ...
    execute: async (_toolCallId, params, signal, _onUpdate, _ctx) => {
      ...
      const worktreeMetadata = buildWorktreeMetadata(record);
      const worktreeReport = worktreeMetadata.worktree || worktreeMetadata.worktreeResult
        ? `Worktree metadata: ${JSON.stringify(worktreeMetadata)}\n\n`
        : "";
      let output =
        `Agent: ${record.id}\n` +
