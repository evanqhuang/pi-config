...
function loadFromDir(dir: string, agents: Map<string, AgentConfig>, source: "project" | "global", strict: boolean): void {
  ...
  for (const file of files) {
    ...
    agents.set(name, {
      ...
      worktreeDisposition: parseWorktreeDisposition(fm.worktree_disposition),
      snapshotSource: parseBoolean(fm.snapshot_source),
...
/** Parse the canonical worktree finalization spelling from frontmatter. */
function parseWorktreeDisposition(val: unknown): WorktreeFinalization | undefined {
  if (val === "commit" || val === "discard") return val;
  return undefined;
}

/** Parse an optional boolean without treating malformed values as false. */
