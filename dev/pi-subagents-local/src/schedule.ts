...
/** Strategy fields are persisted under the same snake_case names used by the
 * Agent tool. Kept local because the on-disk type predates verifier policy. */
interface ScheduledStrategy {
  worktree_disposition?: "commit" | "discard";
...
type RuntimeScheduledSubagent = ScheduledSubagent & ScheduledStrategy;
...
export interface NewJobInput {
  ...
  snapshot_source?: boolean;
  disallowed_tools?: string[];
  /** Camel-case aliases for callers passing resolver output directly. */
  worktreeDisposition?: "commit" | "discard";
  snapshotSource?: boolean;
  disallowedTools?: string[];
}
...
export class SubagentScheduler {
  ...
  buildJob(input: NewJobInput): ScheduledSubagent {
    ...
    const job: RuntimeScheduledSubagent = {
      id: nanoid(10),
      name: input.name,
      ...
      worktree_disposition: input.worktree_disposition ?? input.worktreeDisposition,
      snapshot_source: input.snapshot_source ?? input.snapshotSource,
      disallowed_tools: input.disallowed_tools ?? input.disallowedTools,
      enabled: true,
  ...
  private executeJob(id: string): void {
    ...
    const job = store.get(id) as RuntimeScheduledSubagent | undefined;
    ...
    let agentId: string;
    try {
      // A scheduled job stores the caller's name, not a card snapshot. Reload
      // both card locations at fire time so a file deleted or edited directly
      // in the project cannot leave this timer dispatching a stale process
      // registry. The resolver then applies the current fallbackSubagent
      // policy; an explicit fallback is valid only when that current registry
      // contains one uniquely enabled card.
      registerAgents(loadCustomAgents(ctx.cwd, true));
      ...
      const invocation = resolveAgentInvocationConfig(getAgentConfig(dispatch.type), {
        ...
        worktree_disposition: job.worktree_disposition ?? job.worktreeDisposition,
        snapshot_source: job.snapshot_source ?? job.snapshotSource,
      }, {
      ...
      const disallowedTools = [...new Set([
        ...
        ...(job.disallowed_tools ?? job.disallowedTools ?? []),
      ])];

      // Resolve model at fire time — registry contents may have changed since
      // the job was created (auth added/removed). Fall back silently to the
      // spawn default if resolution fails; the manager still receives the
      // canonical safety strategy below.
      let resolvedModel: any | undefined;
      if (invocation.modelInput) {
        const r = resolveModel(invocation.modelInput, ctx.modelRegistry);
